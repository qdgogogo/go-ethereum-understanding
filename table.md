...
>>>>**bucket(new.sha)**    返回节点对应的桶

>>>>**bumpOrAdd(b, new)**将节点移动到bucket中节点列表的前面，或者如果列表未满，则添加它。 如果n在桶中，则返回值为true。

>>>>>**pushNode(b.entries, n, bucketSize)**添加节点n，并置于首位，返回桶中节点列表和被删除的最后一位

>>**ensureExpirer()**数据库节点超时处理
>>**loop()**维护table的核心逻辑

>>>**doRefresh(refreshDone)**执行随机目标的查找以保持桶满

>>>>**bondall(seeds)**将所有节点进行尝试性绑定，返回成功的节点

>>>>>**bond(false, n.ID, n.addr(), n.TCP)**与该节点进行绑定

>>>>**lookup(tab.self.ID, false)**以自身节点ID为目标查找新的节点

>>>>>**closest(target, bucketSize)**返回距离目标节点最近的16个节点

>>>>>>**push(n, nresults)**将给定节点加入列表，并使其小于最大容量限制
...
add方法试图将节点加入桶中，如果桶中还有空间则立即加入，否则替换掉最近不响应ping的节点
...
doRefresh执行随机目标的查找以保持桶满。如果表为空（初始引导程序或丢弃的故障对等程序），则插入种子节点。
...
bond方法确保本地节点与给定的远程节点具有绑定。如果绑定成功，它还会尝试将节点插入表中。在发送findnode请求之前必须建立绑定。双方必须完成ping/pong交换才能存在绑定。
...
lookup这个函数用来查询一个指定节点的信息。 这个函数首先从本地拿到距离这个节点最近的所有16个节点。然后给所有的节点发送findnode的请求。 然后对返回的节点进行bondall处理, 然后返回所有的节点。
